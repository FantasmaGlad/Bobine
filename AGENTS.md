...
- **Fonctionnalités Clés** :
  ...
  - Correctif layout playlists (Bibliothèque et Coach → Cours Audio) : `VideoPlaylistManager`/`AudioPlaylistManager` sont montés dans `.library-container` APRÈS le bascule `.view-toggle` (et non plus seuls sur une page dédiée comme les anciennes routes `/playlists`/`/audio-playlists`, devenues de simples redirections) — leur `<div>` racine sans classe cassait la chaîne flex (`flex:1`) qui donne sa hauteur au conteneur, écrasant tout le contenu à ~256px (infos qui se chevauchaient, rien de scrollable). Fix en 2 parties : classe `playlist-manager-wrap` sur ce `<div>` racine (`flex:1; min-height:0`) + classe `.library-container.playlists-mode` posée conditionnellement par la page (desktop uniquement, media query `min-width:769px` — le mobile garde son défilement naturel `height:auto`) qui borne la hauteur du conteneur à `calc(100vh - header - 64px)`.
  - Import de dossier complet pour les cours audio (réf. mission "importer un dossier complet et l'avoir sous un même cours") : bouton "Importer un dossier" (`<input webkitdirectory>`, attribut posé sur le noeud DOM car absent du typage React) + glisser-déposer d'un vrai dossier OS géré via `DataTransferItem.webkitGetAsEntry()` et lecture récursive (`collectDroppedAudioFiles`/`readEntryRecursively` dans `app/audio/page.tsx`) — `DataTransfer.files` seul n'expose qu'une entrée dossier vide, illisible directement. Le nom du dossier sert à déduire catégorie/édition via `parseMediaName`, aussi fiable qu'un nom d'archive ZIP.
  - Correctif "cours importés séparément libellisés sous le nom d'un morceau au lieu de Sans catégorie" : en mode Fichiers MP3 (pistes isolées, pas de dossier/ZIP), le mot précédant un nombre dans le nom de la 1ère piste ("Squat 12.mp3") n'a aucune raison d'être une vraie catégorie — l'auto-remplissage de la catégorie ne se déclenche plus que si `parseMediaName` matche une catégorie DÉJÀ existante (`programs`), jamais pour en inventer une nouvelle ; sinon le champ reste vide et le cours atterrit dans le groupe "Sans catégorie" déjà géré par le regroupement dynamique de `/audio`.
  - Correctif critique Android "HDMI branché sur projecteur, `/grid` ne détecte pas l'écran et refuse de lancer un cours" : `wired_display_mode` (le réglage que teste `/grid` pour choisir entre la grille de cours et le placeholder de veille "écran câblé inactif") est une préférence statique en base, jamais dérivée de l'état matériel réel sur AUCUN OS. Android dispose pourtant déjà d'une détection HDMI précise et temps réel (`DisplayManager.DisplayListener` dans `BobineForegroundService.kt`, celle-là même qui pilote `BobinePresentation` pour afficher `/cinema` sur la sortie HDMI) — ce signal n'était simplement jamais remonté au backend. `syncWiredDisplayMode()` fait maintenant un `PUT /api/settings` (best-effort, thread dédié) sur succès de `showPresentationIfNeeded()` (`dual_screen`) et dans `onDisplayRemoved` (`headless`), ce qui déclenche la diffusion WebSocket `display_mode_change` déjà consommée par `/grid`. **Windows/macOS partagent la même lacune structurelle** (aucune énumération native des moniteurs ne dérive `wired_display_mode`, cf. `desktop/tray.py::_get_wired_display_mode`) — non corrigé à ce stade, juste moins visible car ce réglage y est en général choisi une fois lors de l'installation plutôt que rebranché ailleurs comme une tablette. **Décision produit volontaire (2026-09-17)** : ce statu quo Windows/macOS est un choix délibéré, pas un oubli — évalué (implique une énumération native des moniteurs par OS, EnumDisplayMonitors/NSScreen, aucune brique existante dans ce dépôt) et écarté pour l'instant car aucun incident réel n'a été rapporté sur ces OS, contrairement au cas Android (tablette déplacée entre salles/projecteurs). Ne pas "corriger" cet écart de sa propre initiative dans une session future sans qu'un nouveau besoin explicite soit exprimé.
  - Planning visuel (réf. `docs/cahier-des-charges-planning-visuel.md`, CDC complet) : l'ancienne grille calendrier de `/schedule` (chips de taille uniforme, aucune proportionnalité, pas de miniature) est remplacée par une timeline proportionnelle à la durée réelle, façon Google Calendar, avec granularités Jour et Semaine (Semaine masquée sur mobile — 7 colonnes illisibles sous 768px, Jour + Liste y suffisent) et zoom réglable (`ZOOM_LEVELS`, boutons +/-). Miniature par type de cible (`resolve_target_display` côté backend) : vidéo directe, playlist vidéo dérivée de son 1er élément (`Playlist` n'a pas de champ cover, même convention que `RadioPlaylist`), radio via `cover_track_id` (1er morceau ayant une pochette). Bloc radio dimensionné sur sa fenêtre horaire choisie (`time_of_day`→`end_time`), jamais sur le contenu qui boucle ; ambiance 24/7 rendue en bande de fond pleine hauteur (z-index sous les blocs normaux — un bloc au-dessus la masque naturellement, sans calcul de créneaux libres). Chevauchement affiché sans blocage à la création (`computeCutsPrevious`, simule l'ordre d'arrivée runtime : le dernier qui démarre devient la nouvelle fenêtre active) et corrigé côté moteur : `PlaybackManager.state.scheduled_launch` (posé par `load`/`load_playlist` avec `scheduled=True`, préservé à travers l'avance automatique/manuelle d'une playlist) distingue enfin une vraie lecture manuelle d'un reliquat de programmation encore en cours — sans quoi la règle Réseau "le manuel gagne toujours" (`scheduler_manager._launch_target`) annulait aussi à tort un chevauchement programmation-vs-programmation au lieu de laisser la seconde couper proprement la première, sur les 3 canaux (Câblé, Réseau, Radio).
  - Sécurité et hygiène (réf. `docs/cahier-des-charges-securite-hygiene-2026-09.md`, CDC complet) : correctif zip-slip sur les 2 imports ZIP (audio `audio_importer.py`, radio `radio_importer.py`) via un helper commun `app/utils/zip_safety.py::safe_extract_zip` qui rejette toute archive dont un membre résoudrait hors du dossier de destination, avant d'extraire quoi que ce soit. Vérification d'intégrité des mises à jour desktop passée en fail-closed (`update_orchestrator.py::download_with_progress`) : une empreinte SHA-256 introuvable (asset non retrouvé dans la release GitHub) bloque désormais le téléchargement au lieu de laisser l'installation se poursuivre sans vérification. `.github/dependabot.yml` ajouté (npm/pip/cargo/gradle/github-actions, PR groupées par écosystème, revue manuelle, pas d'auto-merge). **Décision produit volontaire (2026-09-17)** : pas de télémétrie de flotte (heartbeat périodique vers un point central pour détecter un appareil client silencieux) — même minimale, elle contredirait le positionnement "local-first, zéro télémétrie" pour un gain opérationnel jugé trop faible (dit qu'un appareil est silencieux, jamais pourquoi). Si le besoin redevient concret, la version qui resterait cohérente avec ce positionnement est documentée dans le CDC §3 : opt-in explicite côté client (bouton visible dans Réglages, jamais actif par défaut), jamais un arrière-plan silencieux. Ne pas réintroduire un heartbeat par défaut sans qu'un nouveau besoin explicite soit exprimé.
