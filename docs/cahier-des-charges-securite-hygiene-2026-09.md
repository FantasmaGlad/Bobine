...
## 5. AGENTS.md / pointeurs Gemini/Claude

### Constat (vérifié le 2026-09-17 — déjà conforme, aucune action requise)

- `.claude/CLAUDE.md` et `.gemini/AGENTS.md` redirigent tous les deux correctement vers `AGENTS.md` racine (contenu vérifié ligne par ligne).
- `AGENTS.md` §3 est à jour, y compris pour des correctifs antérieurs à cette session (vérifié : le fix installeur Windows du commit `9810343`, déjà présent avant que cette session ne commence, y est bien documenté).

Aucun correctif nécessaire pour ce point.
