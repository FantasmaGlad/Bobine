...
**Révision du 2026-09-09, étape 2 — Lot 14 (état final) : l'écran tactile charge maintenant `/grid`, une grille de sélection SEULE, jamais `/cinema`.** `/cinema` ne distinguant aucun rôle primaire/miroir dans son rendu, garder les deux `WebView` sur `/cinema` aurait fait décoder et rejouer le même cours en double dès qu'une vidéo serait réellement lancée (pas encore visible à l'étape 1, aucun cours n'avait été lancé pour de vrai). Nouvelle action `cinema_command`/`"launch"` (+ `video_id`), réutilisant le mécanisme de diffusion déjà existant (`play`/`pause`/`seek`/`stop`) : `/grid` l'émet au clic, `/cinema` (HDMI) la reçoit et joue le cours via son chemin de lecture existant, `/grid` reste affichée sur sa grille en permanence. Validé de bout en bout sur le backend de développement (deux onglets navigateur) ; validation sur la tablette pilote avec un vrai cours encore bloquée par le Lot 8 (aucune vidéo importable sur Android). Détail complet dans les Découvertes du Lot 14 (§16bis).
...
- [~] Lot 14 — séparation `/grid` (écran tactile, sélection seule) / `/cinema` (écran HDMI, lecture) — implémenté et validé de bout en bout sur le backend de dev ; test sur la tablette pilote avec un vrai cours reste bloqué par le Lot 8 (ffmpeg)
