...
Statut (2026-09-09) : **Lots 0, 2, 4, 5, 6, 7 et 13 exécutés** sur **émulateur Android réel** (KVM, x86_64, API 37, écran externe simulé pour le double affichage) — double affichage `/kiosk`+`/cinema` simultané et indépendant, `ForegroundService` survivant à la fermeture de l'app (`dumpsys`), WebSocket fonctionnel, APK signé. **Lot 3 partiellement clos** (routage hostname validé, bascule `cableOutput` bloquée par un bug hors-périmètre `task_67d56a94`, résolu en cours de route au Lot 4). **Lot 13** : publication publique volontairement différée (décision actée).
