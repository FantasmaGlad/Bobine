...
**2. Mur de sécurité Android non documenté par ce plan, non résolu dans cette session — bloquant.** Une fois le binaire réellement trouvé et lancé, `subprocess.run` échoue systématiquement avec `died with <Signals.SIGSYS: 31>` dès qu'on lui demande un vrai travail (lecture de flux vidéo via `ffprobe -show_entries ...` sur un fichier réel). **Preuve que ce n'est PAS un problème du binaire lui-même** : le même fichier `.so`, poussé et exécuté directement via `adb shell` (`adb push` + `adb shell /data/local/tmp/ffprobe_test -version`), fonctionne parfaitement et affiche sa bannière de version complète. La différence est le **contexte d'exécution** : un process lancé par `adb shell` tourne dans le domaine `shell`, alors qu'un `subprocess.run` déclenché depuis l'app tourne dans un process **forké depuis le zygote applicatif**, soumis au filtre **seccomp-bpf spécifique aux apps Android**, plus restrictif que celui du shell — un ensemble bien réel et documenté (AOSP `bionic/libc/SECCOMP_BLOCKLIST`) de syscalls bloqués pour tout process app (`ptrace`, `personality`, `mbind`/`set_mempolicy`/`get_mempolicy`, `process_vm_readv/writev`, etc.), dont certains sont invoqués par le runtime/l'allocateur mémoire de binaires génériques compilés pour un Linux de bureau (glibc) sans jamais avoir prévu de tourner dans un tel bac à sable. Aucun tombstone Android n'est généré pour ce crash précis (`SECCOMP_RET_KILL_PROCESS` semble ne pas déclencher `debuggerd` ici, contrairement à un SIGSEGV classique) — impossible d'identifier l'appel système exact précis sans `strace`/accès root, indisponibles sur cet émulateur non rooté.
...
**Pistes non explorées, à essayer en priorité à la reprise de ce lot** :
- Un build statique **musl** (plutôt que glibc) pourrait éviter l'appel système bloquant en question — le runtime musl est nettement plus minimal et moins susceptible de sonder des fonctionnalités NUMA/exotiques au démarrage. Aucune source musl/ARM64 de confiance équivalente à johnvansickle.com n'a été identifiée dans cette session.
- La solution robuste à terme reste probablement de **compiler ffmpeg nativement pour Android via le NDK** (dans l'esprit de `ffmpeg-android-maker`) : un binaire compilé contre Bionic dès l'origine, par des développeurs conscients de ces contraintes, n'aurait aucune raison d'invoquer les mêmes syscalls de démarrage qu'un build desktop générique. Chantier bien plus lourd (toolchain NDK, cross-compilation autotools) — non tenté ici faute de temps.

**Ce qui reste solide et réutilisable indépendamment du binaire final retenu** : le mécanisme de résolution de chemin (`ffmpeg_binaries.py`, variables d'environnement posées par `bobine_bootstrap.py`) et le fix `useLegacyPackaging` sont corrects et déjà validés (32 tests backend toujours verts, comportement desktop inchangé) — seule la question du **binaire lui-même** reste ouverte. `android/app/src/main/jniLibs/` est gitignored (comme `ffmpeg.exe`/`ffprobe.exe` pour Windows dans `ci.yml`, jamais commités) ; aucune étape CI n'a été ajoutée à `android-build` tant qu'aucun binaire fonctionnel n'est confirmé — inutile de figer un mécanisme de récupération pour un fichier qui ne marche pas.
