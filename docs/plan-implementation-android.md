...
Statut (2026-09-09) : **Lots 0, 2, 4, 5, 6, 7 et 13 exécutés** sur **émulateur Android réel** (KVM, x86_64, API 37, écran externe simulé pour le double affichage) — double affichage simultané et indépendant, `ForegroundService` survivant à la fermeture de l'app (`dumpsys`), WebSocket fonctionnel, APK signé. **Lot 3 partiellement clos** (routage hostname validé, bascule `cableOutput` bloquée par un bug hors-périmètre `task_67d56a94`, résolu en cours de route au Lot 4). **Lot 13** : publication publique volontairement différée (décision actée).
...
**Lots 1, 9, 12, 10, 11 avancés significativement** après la première installation réelle sur la tablette pilote (Xiaomi Pad 8, HyperOS/Android 16) : deux bugs invisibles sur émulateur trouvés et corrigés (icône de lanceur absente ; `ERR_CLEARTEXT_NOT_PERMITTED` bloquant les WebView, corrigé par un `network_security_config.xml` scopé à 127.0.0.1/127.0.0.2) — reste dock USB-C/HDMI physique à tester pour clore le Lot 1. **Lot 9 (stockage)** : `getExternalFilesDir` correctement branché (les données ne vivent plus dans le dossier que Chaquopy redéploie à chaque mise à jour), validé émulateur ; USB OTG probablement déjà couvert par le sélecteur système, non testé faute de périphérique. **Lot 12 (déploiement)** : profil `"android"` explicite dans `deployment.py` (au lieu du repli accidentel `linux-desktop`), confirmé via `/api/settings`. **Lot 10 (Device Owner)** : provisionné et validé sur émulateur (`BobineDeviceAdminReceiver` sans politique + exemption batterie standard), confirmé protéger contre la désinstallation (`DELETE_FAILED_DEVICE_POLICY_MANAGER`) — reste le test de survie 24h et la confirmation HyperOS/MIUI sur la vraie tablette. **Lot 11 (mise à jour in-app)** : `versionCode`/`versionName` dynamiques (CI) + `UpdateManager.kt` (vérification via `/api/updates/check`, téléchargement + installation via `FileProvider`) implémentés et la détection validée en conditions réelles — téléchargement/installation non testables sans un `.apk` public réel (Lot 13).

**Lot 8 (ffmpeg) : cause racine identifiée avec certitude, binaire de remplacement pas encore trouvé.** Le mécanisme de résolution de binaire et `useLegacyPackaging` sont corrects et validés. Diagnostic décisif : le blocage (`SIGSYS`) n'est pas lié à un syscall de codec mais au **runtime de démarrage glibc lui-même** — un `busybox` statique **musl** fonctionne dans les mêmes conditions, tout binaire **glibc** statique meurt avant `main()`. Prochaine étape : trouver/construire un ffmpeg statique **musl** (aucune source prête identifiée — BtbN et martin-riedl.de sont dynamiques, johnvansickle.com est glibc).

**Point secondaire découvert en testant Lot 10 (indépendant, pas un bug introduit)** : Android signale un avertissement « App isn't 16 KB compatible » au premier lancement, listant les libs natives non alignées sur 16 Ko — inclut les libs de Chaquopy elles-mêmes (`libpython3.13.so`, etc.), pas seulement `libffmpeg.so`/`libffprobe.so` du Lot 8. Android bascule automatiquement en « mode compatible », donc pas bloquant, mais à garder en tête si une future contrainte de distribution (Google Play exige la compatibilité 16 Ko depuis fin 2025) devenait pertinente — hors périmètre pour une distribution GitHub Releases directe. Lot 10 non démarré ; Lot 11 reste à faire au-delà du pré-requis de versioning. Ce document est **vivant** : cocher les cases au fur et à mesure, ne jamais le laisser retomber en décalage avec la réalité du dépôt.
...
**Mitigation ajoutée dans une session ultérieure (2026-09-09) : ne résout PAS ce lot, mais débloque l'usage réel en attendant.** Le blocage `SIGSYS` restait entier (aucun binaire musl trouvé/construit), et le premier test réel d'import sur la tablette pilote a confirmé l'échec attendu (`Erreur lors de la lecture des métadonnées : [Errno 2] No such file or directory: 'ffprobe'`). Investigation du code d'import (`backend/app/utils/importer.py`) : `extract_metadata()` (ffprobe) n'était protégée par aucun `try/except` et faisait avorter tout `import_video()`/`import_background()` en cas d'échec, alors qu'aucun champ qui en dépend (`duration_seconds`, `width`, `height`, `codec`, `thumbnail_path`) n'est `NOT NULL` en base, et que `check_compatibility()` traite déjà un `codec`/`audio_codec` absent comme compatible (`is_video_codec_compatible`/`is_audio_compatible` valent `True` si `None`), et que `generate_thumbnail()` a déjà son propre échec toléré dans `import_video()`. Correctif appliqué : `extract_metadata()` est enveloppée (`_extract_metadata_or_unknown`) — un échec dégrade l'import vers des métadonnées inconnues au lieu de l'annuler, seule la détection DRM ne peut alors plus s'exécuter (`is_drm` forcé à `False`). Conséquence concrète : un fichier déjà `.mp4`/H.264 ou HEVC/VP9 + AAC (cas courant d'un export de cours RPM/Sprint/The Trip) s'importe désormais avec succès sous Android **sans jamais appeler ffmpeg ni ffprobe**, seulement dépourvu de durée affichée et de miniature — un fichier nécessitant une vraie normalisation (conteneur non-mp4, codec hors liste blanche) reste, lui, bloqué tant que ce lot n'est pas résolu. Test de non-régression ajouté (`backend/tests/test_video_flow.py::test_07_import_video_without_ffprobe`, rejoue l'erreur exacte observée) ; les 6 tests préexistants restent verts.

**Complément le même jour : la durée, elle, est récupérable sans ffprobe.** `read_mp4_duration_seconds()` (`backend/app/utils/video_utils.py`) lit directement l'atome `mvhd` (imbriqué dans `moov`) d'un MP4/M4V — même style de parcours d'en-têtes de boîtes que `is_faststart_mp4()` juste au-dessus dans le même fichier, aucun appel ffmpeg/ffprobe. `_extract_metadata_or_unknown()` l'appelle en repli quand `extract_metadata()` échoue et que le fichier est `.mp4`/`.m4v`. Largeur/hauteur/codec restent `None` (demanderaient de décoder la boîte `stsd`, hors de portée de ce repli minimal) et **aucune miniature n'est récupérable de cette façon** — une vraie image de la vidéo exige un décodeur vidéo réel (ffmpeg), qu'aucun code Python pur ne peut raisonnablement remplacer. Testé en conditions réelles sur la tablette pilote (cours RPM 108/109/110 réimportés après ce correctif : durées correctes affichées dans `/grid`, ex. « 46 min »). Test ajouté (`test_08_read_mp4_duration_seconds`).

**Miniature manuelle ajoutée le même jour, en compensation de l'absence de miniature auto (retour utilisateur "comment avoir une miniature alors ?")** : nouvel endpoint `PUT /api/videos/{id}/thumbnail` (`backend/app/routers/videos.py`), acceptant un upload d'image (multipart), décodée avec Pillow **uniquement** (même mécanisme que `_import_background_image()` pour un fond fixe — aucun décodage vidéo, donc disponible sur toutes les plateformes, pas seulement Android) : redimensionne à 640×360 max, remplace l'ancienne miniature (fichier nettoyé), met à jour `thumbnail_path`. Interface dans le tiroir d'édition de `frontend/src/app/library/page.tsx` (aperçu + bouton « Changer la miniature », au-dessus du titre du cours). Ne résout pas le Lot 8, contourne seulement l'absence de miniature *automatique* — un admin doit fournir l'image lui-même (capture d'écran, image de la release...). Test ajouté (`test_09_upload_video_thumbnail`, y compris le remplacement et le rejet d'un fichier non-image).
