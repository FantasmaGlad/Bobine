...
1. **Écran tactile de la tablette** : affiche `/grid` (**état final, révision du 2026-09-09 étape 2 — voir §9** : une grille de sélection de cours SEULE, qui ne joue jamais de vidéo localement — une étape intermédiaire de ce même jour avait d'abord essayé `/cinema` directement sur le tactile, dépassée pour la raison expliquée en §9). L'adhérent ou l'encadrant choisit un cours au toucher sur la tablette ; ce choix envoie une commande à distance (`cinema_command`/`"launch"`) sur le canal câblé, et c'est la sortie HDMI qui joue réellement le cours — la tablette affiche ensuite un widget compact pour avancer/reculer/mettre en pause/arrêter ce cours à distance, sans jamais quitter sa grille. La tablette **n'est pas verrouillée en mode kiosque** : elle démarre en plein écran immersif quand l'app Bobine est ouverte, mais un geste système standard (Accueil, multitâche) permet à tout moment de la quitter et d'utiliser la tablette pour autre chose — voir §4.
2. **Sortie HDMI du dock** : affiche `/cinema` (le lecteur plein écran) en 1080p/4K avec décodage matériel via `MediaCodec`, servi en interne par `http://127.0.0.1:8000/cinema` — c'est la « sortie classique » du canal câblé, exactement comme aujourd'hui sur mini PC x86, à une différence près : sur ce profil de déploiement Android, la superposition de commandes locale (retour au menu, play/pause, barre de recherche) n'est plus affichée dessus, remplacée par le widget de la tablette décrit au point 1.
...
| **Mapping des routes** | `/grid` (sélection seule) sur l'écran tactile, `/cinema` (lecture réelle) sur la sortie HDMI, les deux sur le canal câblé `127.0.0.1` — **état final, révisé le 2026-09-09 étape 2** (une étape intermédiaire du même jour avait essayé `/cinema` des deux côtés, avant elle `/kiosk` sur canal réseau indépendant côté tactile ; voir §9) | Choisir un cours au toucher envoie une commande à distance plutôt que de jouer une seconde fois la même vidéo sur la tablette (double décodage/double son évités) — la tablette garde ensuite un widget pour piloter la lecture réelle sur l'écran de la salle |
...
│  │                         │   │                         │  │
│  │ • FastAPI + Uvicorn     │   │ 1. Tablette : /grid      │  │
│  │ • SQLite (database.db)  │   │    (WebView, sélection   │  │
│  │ • APScheduler, zeroconf │   │    seule, widget lecture)│  │
...
- **L'écran tactile de la tablette charge `/grid`, PAS `/cinema`** (état final, révision du 2026-09-09 étape 2, §9) — une grille de sélection seule, sur le même hostname `127.0.0.1`/canal câblé, mais qui ne joue jamais de vidéo localement : `/cinema` ne distingue aucun rôle primaire/miroir dans son rendu, donc les deux `WebView` sur `/cinema` auraient décodé et rejoué la même vidéo en double dès qu'un cours serait réellement lancé (double charge CPU, deux pistes audio) — un problème invisible tant qu'aucun cours n'a été joué pour de vrai, découvert en préparant ce lot. Choisir un cours sur `/grid` envoie une commande à distance (`cinema_command`/`"launch"`) au lieu de jouer quoi que ce soit localement.
- Si aucun écran externe n'est détecté, la `Presentation` HDMI n'est simplement jamais créée — `/grid` reste utilisable pour choisir un cours (l'ordre `"launch"` n'a alors aucun destinataire, best-effort silencieux côté backend), mais rien ne s'affichera nulle part tant qu'aucun écran n'est branché.
...
- **Révision du 2026-09-09, étape 1 (demande explicite utilisateur, validée en conditions réelles sur la tablette pilote avec dock/HDMI physique — DÉPASSÉE par l'étape 2 ci-dessous, gardée pour l'historique) : l'écran tactile de la tablette n'affiche plus `/kiosk` (canal réseau indépendant), mais `/cinema` sur le hostname `127.0.0.1`** — exactement la grille de sélection de cours du canal câblé desktop, sur le **même canal** que la sortie HDMI. `MainActivity.kt` a changé (`KIOSK_URL` → `CINEMA_URL`), aucun changement backend.
- **Révision du 2026-09-09, étape 2 — ÉTAT FINAL (Lot 14 puis 16ter, `docs/plan-implementation-android.md` §16bis/§16ter), validée avec de vrais cours sur la tablette pilote.** `/cinema` ne distinguant aucun rôle primaire/miroir dans son rendu, garder les DEUX `WebView` dessus aurait fait décoder et rejouer le même cours en double (double charge CPU, deux pistes audio) dès qu'un cours serait réellement lancé — invisible à l'étape 1 tant qu'aucune vidéo n'avait été jouée pour de vrai. **L'écran tactile charge maintenant `/grid`** (`GRID_URL`, `MainActivity.kt`) — une grille de sélection SEULE, qui ne joue jamais de vidéo localement et ne quitte jamais son propre affichage. Choisir un cours au toucher envoie une commande `cinema_command`/`"launch"` (+ `video_id`) sur le canal câblé — nouvelle action ajoutée au mécanisme de commande à distance déjà existant (`play`/`pause`/`seek`/`stop`, utilisé jusqu'ici par le tableau de bord admin desktop). **C'est la sortie HDMI (`BobinePresentation`, restée sur `/cinema`, inchangée) qui joue réellement le cours.** `/grid` affiche ensuite un widget compact (« en cours de lecture ») pour avancer/reculer/mettre en pause/arrêter le cours à distance, réutilisant le même mécanisme `cinema_command` — et la superposition de commandes historiquement affichée SUR l'écran HDMI a été retirée **spécifiquement pour ce profil de déploiement** (`deployment_profile === "android"`, exposé par `/api/settings` depuis le Lot 12), devenue redondante avec ce widget. Cette restriction ne s'applique volontairement PAS aux installations desktop en libre-service (pas de `/grid` chez elles) : `/cinema` y garde sa superposition de commandes locale, inchangée. `/kiosk` et le hostname `127.0.0.2` restent fonctionnels et documentés (§4) mais ne sont plus utilisés par défaut sur Android.
