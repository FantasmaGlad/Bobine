...
- **Risque de compatibilité, VALIDÉ EMPIRIQUEMENT (Lot 0 du plan exécuté le 2026-09-08, pas seulement planifié)** :
  ...
  - **Décision actée et validée par build réel : downgrade Pydantic v1 sur le profil Android uniquement.** `pydantic==1.10.26` (vraie librairie pure Python, testée) + `fastapi==0.99.1` (dernière version encore compatible Pydantic v1) — build APK complet réussi avec cette combinaison. Périmètre de compatibilité dans le code mesuré par grep (pas estimé) : 3 points localisés — `pydantic_settings.BaseSettings` → `pydantic.BaseSettings` intégré ([`config.py:6`](../backend/app/config.py)), `field_validator` → `validator` ([`radio_announcements.py:20,75`](../backend/app/routers/radio_announcements.py)), `.model_dump()` → `.dict()` ([`settings.py:339`](../backend/app/routers/settings.py)). Le reste du backend n'utilise que `BaseModel` nu, compatible sans changement. **Compromis assumé** : le profil Android reste figé sur ce millésime FastAPI/Pydantic (mi-2023) tant qu'aucune distribution Android de `pydantic-core` n'existe — toute évolution future du backend touchant Pydantic doit rester compatible avec les deux syntaxes. Détail complet et log exact dans les Découvertes du Lot 0 ([`docs/plan-implementation-android.md`](plan-implementation-android.md)).
...
En revanche, la `WebView` de l'écran tactile **ne doit pas** aussi tourner sur `127.0.0.1`, sinon elle serait elle aussi classée « câblé » et suivrait le même réglage — cassant l'indépendance entre « ce que l'adhérent sélectionne au toucher » et « ce qui joue à l'écran ». Elle doit charger `/kiosk` via un hostname distinct — `bobine.local` (résolu localement via le service mDNS que le backend publie déjà, §5) ou l'adresse IP LAN de la tablette elle-même. Le Lot dédié du plan d'implémentation tranche entre ces deux options et l'implémente ; ce cahier des charges ne fige pas ce choix, seulement la contrainte qu'il doit respecter.
